...
function LoadConfig() {
	...
	*/
	...
	Config.HealStatus = false; // (解中毒詛咒)Go to a healer if poisoned or cursed
	...
	Config.UseHP = 75; // (生命低於喝水)Drink a healing potion if life is under designated percent.
	...
	Config.UseMercRejuv = 0; // (傭兵生命低於喝紫水)Give a rejuvenation potion to your merc if his/her life is under designated percent.
	...
	Config.LifeChicken = 30; // (生命小於退出)Exit game if life is less or equal to designated percent.
	...
	Config.FastPick = false; // (是否在攻擊時進行拾取。)Check and pick items between attacks
	...
	Config.PublicMode = 2; // (組隊設定:1為發送或接受組隊邀請,2為僅接受組隊邀請,3為僅發送組隊要求,0為關閉組隊功能)1 = invite and accept, 2 = accept only, 3 = invite only, 0 = disable
	...
	Config.MinGameTime = 60; // (最短遊戲時間/秒)Min game time in seconds. Bot will TP to town and stay in game if the run is completed before.
	Config.MaxGameTime = 0; // (最長遊戲時間/秒)Maximum game time in seconds. Quit game when limit is reached.
	...
	Config.BossPriority = false; // (是否優先擊殺金怪)Set to true to attack Unique/SuperUnique monsters first when clearing
	Config.ClearType = 0xF; // (清場模式：0xF為跳過普通怪物,0為所有怪物全部清理)Monster spectype to kill in level clear scripts (ie. Mausoleum). 0xF = skip normal, 0x7 = champions/bosses, 0 = all
